...
describe("core version compatibility", () => {
  it("keeps declared version line stable", () => {
    ...
    expect(CORE_VERSION_SERIES).toBe("0.5.x");
  });

  it("accepts equal or lower minimum versions", () => {
    expect(isCoreVersionCompatible()).toBe(true);
    expect(isCoreVersionCompatible("0.5.0")).toBe(true);
    expect(isCoreVersionCompatible("0.5.x")).toBe(true);
    expect(isCoreVersionCompatible("0.4.13")).toBe(true);
  });

  it("rejects higher or invalid minimum versions", () => {
    expect(isCoreVersionCompatible("0.6.0")).toBe(false);
