...
describe("core version compatibility", () => {
  it("keeps declared version line stable", () => {
    ...
    expect(CORE_VERSION_SERIES).toBe("0.2.x");
  });
  ...
  it("accepts equal or lower minimum versions", () => {
    ...
    expect(isCoreVersionCompatible("0.2.x")).toBe(true);
    expect(isCoreVersionCompatible("0.1.9")).toBe(true);
  });

  it("rejects higher or invalid minimum versions", () => {
    expect(isCoreVersionCompatible("0.3.0")).toBe(false);
