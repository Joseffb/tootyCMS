...
function tableName() {
  return `${prefix()}domain_events_queue`;
}
...
export async function ensureDomainQueueTable() {
  if (ensured) return;
...
export async function claimDomainEventBatch(limit = 25) {
  await ensureDomainQueueTable();
  const table = sql.raw(quotedTableName());
...
export async function markDomainEventProcessed(id: string) {
  const table = sql.raw(quotedTableName());
  await db.execute(sql`
    UPDATE ${table}
...
export async function markDomainEventFailed(id: string, attempts: number, errorMessage: string) {
  const table = sql.raw(quotedTableName());
  const maxAttempts = 8;
