...
export default async function CategoryArchivePage({ params }: { params: Params }) {
  ...
  const data = await getTaxonomyArchiveData(decodedDomain, "category", decodedSlug);
  if (!data) notFound();
  ...
  if (site?.id) {
    const categoryTemplate = await getThemeTemplateByHierarchy(site.id, {
      taxonomy: "category",
      slug: decodedSlug,
    });
    if (categoryTemplate) {
  ...
  return (
    <main
      ...
      data-theme-route-kind="category_archive"
          ...
          <div className="mt-4 grid items-center gap-6 md:grid-cols-[1.2fr_0.8fr]">
            <div>
              ...
              <p className="tooty-post-description mt-4 max-w-3xl text-lg font-medium md:text-xl">
                {data.posts.length} published {data.posts.length === 1 ? "post" : "posts"} in this category.
              </p>
            </div>
