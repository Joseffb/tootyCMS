...
- Current notes:
  ...
  - validation completed:
    ...
    - GitHub Actions follow-up: `tests/extension-paths.test.ts` no longer assumes a checked-in `.env` or a workstation-specific repo root, so CI runners now match the runtime config fallback behavior
    - `npm run test`: passed
    - `npm run test:integration`: passed
