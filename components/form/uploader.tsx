...
export default function Uploader({
  ...
  const handleUpload = async (file: File | null) => {
    if (file) {
      ...
      try {
        const result = await uploadSmart({ file, siteId, name });
        if (result) {
          setData((prev) => ({ ...prev, [name]: result.url }));
          toast.success("Image uploaded successfully");

          const hiddenInput = document.querySelector<HTMLInputElement>(`input[name="${name}Url"]`);
          if (hiddenInput) hiddenInput.value = result.url;

          const finalInput = document.querySelector<HTMLInputElement>(`input[name="${name}FinalName"]`);
          if (finalInput) finalInput.value = result.url;
        } else {
  ...
  return (
    <div>
      ...
      >
        ...
        />
        <div
          className={`${dragActive ? "border-2 border-black" : ""} absolute z-[3] flex h-full w-full flex-col items-center justify-center rounded-md px-10 transition-all ${
            data[name]
      ...
      <div className="mt-1 flex rounded-md shadow-sm">
        ...
        />
        {/* 🔽 Inject hidden fields for server action */}
        <input type="hidden" name={`${name}Url`} />
        <input type="hidden" name={`${name}FinalName`} />
      </div>
    </div>
  );
}
